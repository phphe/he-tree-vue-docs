...
```
The package has only a few simple css. The Fold plugin and Check plugin have only functional functions, no UI, and requires your ui. The Tree component has a default slot to customize the node UI, and `node.text` is displayed by default when no slot template passed in. If you create a component that inherits from Tree, you can also use jsx and hook functions to customize the template. This guide will list many usage demos and source code.
...
## Usage
If you need use tree in multiple places, you can [create a component that inherits from Tree](#customize).
```vue
<!-- Demo1Easiest.vue -->
<template>
...
`triggerClass` is `tree-node` by default, so clicking anywhere inside the node can trigger dragging except in special cases. Modify `triggerClass` to specify the element that triggers dragging. prop `triggerBySelf` can prevent child elements from triggering drag.
...
Some events are triggered in the process. Events cannot affect the process. Hook methods can affect the process. `drag` is triggered at the beginning. `before-drop` is triggered before drop completed. The tree will trigger `input`, `change` if structure changed by drag. Trigger `drop` after drop completed.
...
Try dragging the nodes below to the edge of the box and the edge of the page.
...
## RTL
Display and drag from right to left. Use prop to `rtl` control that.
